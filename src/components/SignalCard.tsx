            ...
            <div className="flex items-center justify-between gap-2 mb-4">
                <div className="flex items-center gap-3 min-w-0">
                    {image && !imgError ? (
                        ...
                        <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-[var(--accent-cyan)] to-[var(--accent-purple)] flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                            {coin.slice(0, 3)}
                        </div>
                    )}
                    <div className="min-w-0">
                        <div className="font-semibold text-base sm:text-lg truncate">{name}</div>
                ...
                <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
                    ...
                    <button
                        ...
                        className="p-1.5 sm:p-2 rounded-lg text-slate-400 hover:text-[var(--accent-cyan)] hover:bg-slate-50 transition-all"
                    ...
                    <button
                        ...
                        className={`p-1.5 sm:p-2 rounded-lg transition-all ${isWatched
                            ? 'text-amber-500 hover:bg-amber-50'
                            : 'text-slate-300 hover:text-amber-400 hover:bg-slate-50'
                            }`}
                        title={isWatched ? 'Remove from watchlist' : 'Add to watchlist'}
                    >
                        <svg className="w-5 h-5 sm:w-6 sm:h-6" fill={isWatched ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                        </svg>
                    </button>
                    <div className={`score-badge ${getScoreClass()} border border-black/30 text-3xl sm:text-4xl px-3.5 py-2 sm:px-5 sm:py-3 font-bold`} style={{ WebkitTextStroke: '0.5px black' } as React.CSSProperties}>{score}</div>
                </div>
            </div>
